#!/usr/bin/env python3
"""
Create HuggingFace dataset with multiple Parquet files in data/ directory.

This version creates the standard HuggingFace Hub structure with:
- data/train-XXXXX-of-XXXXX.parquet files
- README.md with proper dataset_info
"""
...
class ParquetDatasetConverter:
    """
    Creates HuggingFace dataset with Parquet files in data/ directory.
    """
    ...
    def collect_all_data(self) -> List[Dict[str, Any]]:
        """Collect all manuscript data and structure it according to the schema."""
        ...
        for i, letter_dir in enumerate(letter_dirs):
            ...
            try:
                ...
                for img_path in original_images:
                    ...
                    # Create record matching the desired schema
                    record = {
                        'id': f"{letter_dir.name}_{page_number}",
                        'manuscript_id': letter_dir.name,
                        'page_id': page_number,
                        'line_id': "full_page",  # Since we have full page transcriptions
                        'transcription': text_data['abbreviated'],  # What appears on the image
                        ...
                        'bbox_x': 0,  # Full page bounding box
                        'bbox_y': 0,
                        'bbox_width': pil_image.size[0],
                        'bbox_height': pil_image.size[1],
                        'baseline': "",  # Not applicable for full page
                        'language': "latin",
                        'script': "gothic",
                        'type': "synthetic",
                        ...
                        'rdf': "",
                        'image': pil_image,
                        'full_page_image': pil_image,  # Same as image for full page
                    }
    ...
    def create_dataset(self, data: List[Dict[str, Any]]) -> Dataset:
        ...
        # Define features to match the schema
        features = Features({
            'id': Value('string'),
    ...
    def save_parquet_dataset(self, dataset: Dataset, num_shards: int = 5):
        """Save dataset as multiple Parquet files in data/ directory."""
        ...
        # Create README.md
        self.create_readme(dataset, total_bytes, num_shards)
...
A synthetic dataset of medieval manuscript pages with original and abbreviated transcriptions.

## Dataset Description

This dataset contains {num_examples} synthetic medieval manuscript pages featuring:

- **High-quality synthetic manuscript images** (375x500px resolution)
- **Original transcriptions** before abbreviation application
- **Abbreviated transcriptions** as they appear on the manuscript pages
- **Medieval Latin text** with authentic abbreviation rules
- **Gothic script styling** with realistic handwriting variations
- **Authentic textures** and ink color variations

## Features

- `transcription`: Text as it appears on the manuscript (with abbreviations like "omn;" for "omnibus")
- `full_page_transcription`: Original text before abbreviation rules were applied
- `image`: The synthetic manuscript page image
- `manuscript_id`: Unique identifier for each letter
- `page_id`: Page number within the manuscript
- Additional metadata fields for compatibility with other manuscript datasets
...
- **OCR system development** for historical documents
- **Medieval paleography research**
- **Abbreviation expansion studies**
- **Historical document digitization benchmarks**
...
@dataset{{medieval_letters_synthetic,
  ...
  author={{Synthetic HTR Generator}},
  year={{2024}},
  description={{Synthetic medieval manuscript dataset with original and abbreviated transcriptions}},
  license={{CC-BY-4.0}}
}}
```
"""
    ...
    def convert(self):
        ...
        print("📦 Parquet Dataset Converter")
        print("=" * 50)
        ...
        print("\n🎉 Parquet dataset creation completed!")
        print("   ✅ Multiple Parquet files in data/ directory")
        print("   ✅ Proper README.md with dataset_info")
        print("   ✅ Ready for HuggingFace Hub upload!")


def main():
    parser = argparse.ArgumentParser(description="Create Parquet dataset with data/ directory structure")
    ...
    try:
        converter = ParquetDatasetConverter(args.input_dir, args.output_dir)
        converter.convert()
    except Exception as e:
        print(f"❌ Error: {e}")
        import traceback
